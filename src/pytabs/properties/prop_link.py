...
class PropLink:
    ...
    def set_linear(self, name: str, dof: list[bool], is_fixed: list[bool],
                   ke: list[float], ce: list[float], dj2: float, dj3: float,
                   ke_is_coupled: bool = False, ce_is_coupled: bool = False,
                   notes: str = '', guid: str = '') -> None:
        [ret, _dof, _is_fixed, _ke, _ce] = self.prop_link.SetLinear(name, dof, is_fixed, ke, ce,
                                                                    dj2, dj3, ke_is_coupled, ce_is_coupled,
                                                                    notes, guid)
        handle(ret)
        
    def set_multi_linear_elastic(self, name: str, dof: list[bool], is_fixed: list[bool],
                                 nonlinear: list[bool], ke: list[float], ce: list[float],
                                 dj2: float, dj3: float, notes: str = '', guid: str = '') -> None:
        [ret, _dof, _is_fixed, _nonlinear, _ke, _ce] = self.prop_link.SetMultiLinearElastic(name, dof, is_fixed, nonlinear,
                                                                                            ke, ce, dj2, dj3, notes, guid)
        handle(ret)
        
    def set_multi_linear_plastic(self, name: str, dof: list[bool], is_fixed: list[bool],
                                 nonlinear: list[bool], ke: list[float], ce: list[float],
                                 dj2: float, dj3: float, notes: str = '', guid: str = '') -> None:
        [ret, _dof, _is_fixed, _nonlinear, _ke, _ce] = self.prop_link.SetMultiLinearPlastic(name, dof, is_fixed, nonlinear,
                                                                                            ke, ce, dj2, dj3, notes, guid)
        handle(ret)
