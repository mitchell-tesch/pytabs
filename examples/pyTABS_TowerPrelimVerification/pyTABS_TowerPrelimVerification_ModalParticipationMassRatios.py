...
WB_MODAL_RESULTS_OUT = 'O23'

def extract_input_data (modal_participating_mass_ratios_wb : xl.Book) : 
    ...
    Returns: 
        _type_ : input data 
    """
    input_sheet = modal_participating_mass_ratios_wb.sheets[WB_MODAL_IN_SHEET]
...
def modal_participating_mass_ratio_results_data (model_fp, model_is_open, modal_case): 
    ...
    modal_participating_mass_ratios = etabs_model.analysis_results.modal_participating_mass_ratios()

    # Generating a modal period dataframe from Etabs values 
    modal_participating_mass_ratio_df = pd.DataFrame(modal_participating_mass_ratios)

    # get first five mode shapes. 
    modal_participating_mass_ratio_df_2 = modal_participating_mass_ratio_df.drop(range(5, len(modal_participating_mass_ratio_df.index.to_list())))
    return modal_participating_mass_ratio_df_2
...
def main(): 
    ...
    model_modal_participating_mass_ratio_wb = xl.Book.caller() 
    header_input = extract_input_data(model_modal_participating_mass_ratio_wb)
    ...
    results = modal_participating_mass_ratio_results_data(model_fp, model_is_open, modal_case)

    print('\nWriting to excel...', end='')

    print('Returning modal results...')
    write_batch_results(model_modal_participating_mass_ratio_wb, results)
